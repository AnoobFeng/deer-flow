...
**Trace-attribute injection points**: both `runtime/runs/worker.py::run_agent` (gateway path) and `client.py::DeerFlowClient.stream` (embedded path) merge the metadata into `config["metadata"]` right before constructing the graph. `subagents/executor.py::_aexecute` does the same for every subagent run so subagent traces group under the parent thread's session card (carrying the parent `thread_id` → `langfuse_session_id`, the user_id captured at `task_tool` → `langfuse_user_id`, and a `subagent:<normalized-name>` trace name). Standalone model calls that intentionally remain separate traces also inject the same reserved metadata and wrap the LLM call in `langfuse_trace_attribute_context()` when their owning boundary knows the thread/user: `MemoryUpdater` uses the queued memory context's `thread_id`/`user_id`, and `app.gateway.routers.suggestions` uses the route `thread_id` plus the effective user id. Caller-supplied keys win via `setdefault`, so an external `session_id` override is preserved. Field mapping:
