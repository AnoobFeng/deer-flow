...
from deerflow.tracing import DEERFLOW_TRACE_ID_METADATA_KEY
...
class MemoryMiddleware(AgentMiddleware[MemoryMiddlewareState]):
    ...
    def after_agent(self, state: MemoryMiddlewareState, runtime: Runtime) -> dict | None:
        ...
        if runtime.context:
            value = runtime.context.get(DEERFLOW_TRACE_ID_METADATA_KEY)
            if isinstance(value, str) and value:
                deerflow_trace_id = value
        if deerflow_trace_id is None:
            runtime_config = getattr(runtime, "config", None)
            metadata = runtime_config.get("metadata", {}) if isinstance(runtime_config, dict) else {}
            value = metadata.get(DEERFLOW_TRACE_ID_METADATA_KEY) if isinstance(metadata, dict) else None
            if isinstance(value, str) and value:
                deerflow_trace_id = value
