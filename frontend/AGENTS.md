...
3. `useThreadHistory` loads persisted conversation pages from `GET /api/threads/{id}/messages/page`, preserving the backend's thread-global event `seq`; rendering merges persisted pages, the checkpoint/live tail, then optimistic/transient messages without timestamp re-sorting
