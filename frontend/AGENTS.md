...
3. `useThreadHistory` loads persisted conversation pages from `GET /api/threads/{id}/messages/page`, preserving the backend's thread-global event `seq`; rendering overlays checkpoint/live copies at their matching canonical identities (a summarized checkpoint may contain a protected early input plus a recent tail), uses retained transient identity anchors to place compression rescues across newest-first page boundaries, then adds optimistic messages without timestamp re-sorting
